...
const resultBackRule = css.match(/\.equity-unknown-hand \.poker-card-back\s*\{[^}]+\}/)?.[0] ?? '';
...
test('Equity DOM card backs own a bounded positioned card box', () => {
  ...
  assert.match(resultBackRule, /width:\s*var\(--card-size-standard-width\)/);
  assert.match(resultBackRule, /height:\s*var\(--card-size-standard-height\)/);
  assert.match(resultBackRule, /flex:\s*0 0 auto/);
});
...
test('shared Equity scenario cards continue to use the accepted face renderer', () => {
  assert.match(logic, /training-readonly-card riverline-card/);
  assert.match(logic, /\$\{cardMarkup\(card\)\}/);
  assert.match(css, /\.equity-scenario \.training-readonly-card\s*\{\s*width:\s*var\(--card-size-result-width\);\s*height:\s*var\(--card-size-result-height\)/);
});
