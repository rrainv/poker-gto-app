...
test('DOM cards use paired in-bounds corners and full-ten has a local optical treatment', () => {
  ...
  assert.match(css, /\.card-slot \.card-corner \.rank--ten,[\s\S]*?letter-spacing:\s*-\.1em/);
...
test('table cards expose bounded style variants without changing table geometry', () => {
  ...
  assert.match(css, /\.riverline-card-tournament-rank\.table-card-rank--ten \{[^}]*17px/);
  ...
  for (const [x, y] of [[6, 15], [6, 28], [5, 13], [5, 25], [20, 25], [20, 42]]) {
...
test('Clean Corner keeps its primary table corner while omitting the secondary markup', () => {
  ...
  assert.match(renderCard, /table-card-corner--top/);
  assert.match(renderCard, /riverline-card-rank table-card-rank\$\{rankClass\}/);
  assert.match(renderCard, /riverline-card-suit table-card-suit/);
