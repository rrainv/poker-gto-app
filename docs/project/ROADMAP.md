...
Last refreshed: August 31, 2026 (`DECISION-ECONOMICS-001` is human accepted; `STRATEGY-TRUST-001` now leads the remaining foundational gates).
...
20. **COMPLETED / HUMAN ACCEPTED DECISION ECONOMICS — `DECISION-ECONOMICS-001`.** Canonical ledger-pot accounting was correct; actor-relative strategic pricing is now exact where canonical evidence exists and unavailable rather than total-pot-derived where it does not. **ACTIVE NEXT:** `STRATEGY-TRUST-001`, then `DECISION-CONTEXT-SINGLE-AUTHORITY-001`.
...
Completed `TABLE-PHYSICALITY-003`, `HOME-GAME-001B`, `SETTINGS-IA-001`, and the bounded `HANDS-ON-DEFECTS-001` repair remain useful visible-product foundations. Core Flow, Hand Replay, the all-in runout regression repair, Training composition, Equity, Analyze/Range, First Use/Home, Saved Visual Knowledge, and the workflow-first Guide remain accepted checkpoints. The completed audits found foundational debt beneath some checkpoint claims. Training Memory ownership and actor-relative decision economics are now repaired and accepted; DecisionContext convergence, strategy trust and grading, generalized lifecycle disposal including durable anonymous Device Guest, portable browser testing, Saved retrieval, and root composition remain. Those owners now lead; no broad polish or expansion wave begins first.
