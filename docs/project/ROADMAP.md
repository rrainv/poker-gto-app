...
Last refreshed: August 30, 2026 (`SAVED-VISUAL-KNOWLEDGE-001` is **COMPLETED / HUMAN ACCEPTED**; the observed all-in runout regression is active next, then `GUIDE-CONTENT-001`, then September Alpha closure/audit preparation before evidence-based activation of another major feature).
...
17. **ACTIVE NEXT — OBSERVED ALL-IN RUNOUT REGRESSION —** reproduce and repair the observed progression/presentation regression while preserving canonical flop → turn → river → showdown chance events and accepted Hand/Replay architecture.
18. **PLANNED NEXT — `GUIDE-CONTENT-001`** — current, visual, task-oriented Guide and concise human explanatory content through the existing help/tutorial authority.
19. **SEPTEMBER ALPHA CLOSURE GATES —** human whole-app pass → documentation integrity/continuity freeze → human clean commit/audit tag.
20. **INDEPENDENT CROSS-MODEL DEEP AUDIT —** blind hands-on product audit plus white-box theoretical/architecture audit → human triage → fixes for serious P0/P1 findings.
21. **PRESERVED MAJOR FEATURES / POST-AUDIT ACTIVATION DECISION —** `PERSONAL-STRATEGY-003A`, `GAME-SETUP-EVOLUTION-001`, `RANGE-EVOLUTION-001`, or another major documented capability may be activated only from audit evidence and human prioritization.
22. **PLANNED LATER / PRE-RELEASE QUALITY GATE — `PREMIUM-CLOSEOUT-001`** — whole-product release-quality acceptance after nearer visible owners; it is not the September Alpha milestone.
23. **Release/mobile/social/PLO later** — only after their prerequisites and explicit decisions.

## Phase A — usable visible-product foundation

Completed `TABLE-PHYSICALITY-003`, `HOME-GAME-001B`, `SETTINGS-IA-001`, and the bounded `HANDS-ON-DEFECTS-001` repair remain useful visible-product foundations. Core Flow, Hand Replay, Training, Equity, Analyze/Range, First Use/Home, and Saved Visual Knowledge are completed human-accepted checkpoints. The observed all-in runout regression is next, followed by Guide content and September Alpha closure/audit preparation. This does not claim perfect polish or whole-product visual acceptance.
