...
Last refreshed: August 28, 2026 (`TRAINING-COMPOSITION-001` is **COMPLETED / HUMAN ACCEPTED**; `EQUITY-COMPOSITION-001` is **NEXT / READY**, followed by visible-product closure, bounded secondary polish, and the September Alpha whole-app/docs/audit gates before evidence-based activation of another major feature).
...
13. **NEXT / READY — `EQUITY-COMPOSITION-001`** — bounded player tiles, optional names, central Board/Dead/Method, and dominant results.
...
Completed `TABLE-PHYSICALITY-003`, `HOME-GAME-001B`, `SETTINGS-IA-001`, and the bounded `HANDS-ON-DEFECTS-001` repair remain useful visible-product foundations. Core Flow, Hand Replay, and Training are completed human-accepted checkpoints. Equity is next and ready, followed by Analyze/Range UX and First Use/Home before the bounded secondary polish wave. This does not claim perfect polish or whole-product visual acceptance.
