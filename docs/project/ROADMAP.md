...
Last refreshed: September 1, 2026 (`AUDIT-CHEAP-FIX-BATCH-001` is accepted; `AUTH-SUPABASE-SINGLETON-001` now leads the remaining foundational gates).
...
22. **ACTIVE NEXT — `AUTH-SUPABASE-SINGLETON-001`** — establish bounded singleton ownership for the Supabase/auth client without absorbing the broader identity lifecycle.
23. **IDENTITY AND PRODUCT-TRUTH GATES —** `IDENTITY-LIFECYCLE-001` → `HEURISTIC-BASELINE-TRUTH-001` → `TRAINING-NORMATIVE-001`.
24. **PORTABLE TEST/LIFECYCLE PLATFORM — `BROWSER-TEST-PLATFORM-001`.**
25. **BOUNDED UI COMPOSITION EXTRACTION —** `UI-COMPOSITION-ROOT-001` → one workspace extraction pilot. This is risk-driven incremental decomposition, not React/Redux or a framework rewrite.
26. **CURRENT SAVED RETRIEVAL — `SAVED-LIBRARY-001`** — full current Hand/Spot retrieval before any new Saved payload kind.
27. **STUDY CONTINUITY — `HOME-STUDY-CONTINUITY-001`.**
28. **BOUNDED REFERENCE-SOURCE DECISION —** after read-only source research, which may run in parallel.
29. **LEARNING-LOOP EXPANSION.**
30. **PERSONAL STRATEGY — `PERSONAL-STRATEGY-003A`.**
31. **PLANNED LATER / PRE-RELEASE QUALITY GATE — `PREMIUM-CLOSEOUT-001`** — whole-product release-quality acceptance after nearer foundational owners.
32. **Release/mobile/social/PLO later** — only after their prerequisites and explicit decisions.
