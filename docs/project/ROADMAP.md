...
Last refreshed: August 27, 2026 (`CORE-FLOW-CORRECTNESS-001` completed human-QA correction #1 and requires final human acceptance; Replay rail remains planned next but blocked on that acceptance, followed by Training composition and `PERSONAL-STRATEGY-003A` before a whole-app mini-pass).
...
10. **IMPLEMENTATION COMPLETE / FINAL HUMAN ACCEPTANCE REQUIRED — `CORE-FLOW-CORRECTNESS-001`** — completed Hand to fresh Hand with two primary terminal actions/new identity, session-level append-only correction over all eligible ledger types with optional reason, nested known-opponent picker Escape, and canonical min-raise verification are implemented and pass focused Node plus Firefox 154 EN/RU/HE/RTL interaction checks. Final product-owner acceptance remains open.
11. **PLANNED NEXT / BLOCKED ON CORE FLOW HUMAN ACCEPTANCE — `REPLAY-RAIL-NAV-001`** — vertical street-grouped history, first-class rail actions and Current Legal Actions, compact Hand Stage, coherent cards/seat/player ownership, folded-seat readability, physical Dealer placement, contribution-line clarity, table/history/action integration, and timeline typography.
...
Completed `TABLE-PHYSICALITY-003`, `HOME-GAME-001B`, `SETTINGS-IA-001`, and the bounded `HANDS-ON-DEFECTS-001` repair remain useful visible-product foundations. The newly discovered Core Flow lifecycle/input issues are implementation complete with focused and real-Firefox agent evidence, but still require the explicit human checkpoint; structural Replay work must not start before it. Repeated local Welcome/card/warning polishing stops: launch semantics move to `FIRST-USE-HOME-001`, warning presentation to `HOME-GAME-PRESENTATION-001`, and coherent table/card/seat/rail design to `REPLAY-RAIL-NAV-001`. The checkpoints do not close broader HPR findings.
