...
Last refreshed: August 23, 2026 (`RETURN-QUEUE-001`, current through `PREFLOP-CALIBRATION-001`).
...
|---|---|---|---|---|
| RET-TABLE-001 | RETURN-SOON | Hand / Table Presence | Existing canonical table works, but table identity/stack/card overlap hierarchy, adaptive player-count geometry, live-vs-complete composition, and table/support-rail visual acceptance remain incomplete. | `TABLE-PRESENCE-REF-001` → `TABLE-PRESENCE-002` |
| RET-FULLHAND-001 | RETURN-SOON | Full Hand / Replay | Canonical replay exists, but the visual hand timeline, concise hand-complete state, and post-hand Review/Replay/Repeat/Save/Next flow are not yet product-finished. | `TABLE-PRESENCE-002`, then `FULL-HAND-REVIEW-001` if needed |
...
| RET-REFERENCE-UI-001 | RETURN-BEFORE-SUBSYSTEM-DONE | Strategy source authority | Claim-policy semantics are automated, but Training pre/post answer wording, Full Hand comparison copy, high-risk notes, Playbook provenance, Matrix precision, Daylight/Midnight and HE RTL still need human Firefox acceptance. | Reference-authority human acceptance; bundle into premium/full-hand passes where appropriate |
