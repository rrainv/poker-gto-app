...
Last refreshed: August 25, 2026 (`DOCS-CAPABILITY-DOSSIERS-001` completed; `TABLE-PHYSICALITY-003` is active next).
...
| RET-CARDS-THEMES-001 | RETURN-SOON | Cards / Themes / Layout / Density | `UX-REGRESSION-001` accepted picker/known-card legibility, and `WORKSPACE-COMPOSITION-002` repaired the visible rank+suit floor without exposing hidden cards, removed weak layout choices, and retired the low-value density setting while retaining harmless internal foundations. Independent final human known-card suit verification remains open. | `RET-COMPOSITION-002` human card verification, `SETTINGS-IA-001`, then closeout |
