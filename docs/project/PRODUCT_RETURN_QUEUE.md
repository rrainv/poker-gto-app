...
Last refreshed: August 24, 2026 (`DOCS-INTEGRITY-001`; current hands-on debt reconciled).
...
|---|---|---|---|---|
| RET-UX-REGRESSION-001 | RETURN-SOON | Cross-product regressions | Current hands-on blockers include card-picker legibility, human sizing precision, one-row board, tutorial persistence, Home Game Create routing, theme edit semantics where regressed, bookmark consistency, Training 3+1 control layout, street-aware hints, Table Focus legibility, and rank+suit identity for every known card. Detailed facts remain `QA-HANDSON-001/002/004/006/008/012/016/018/019/020/021`. | `UX-REGRESSION-001` immediately after Docs Integrity |
| RET-COMPOSITION-002 | RETURN-SOON | Workspace composition | Controls First/Equity dead space, weak presets, weak density value, equal-weight Training context boxes, table balance, and default physical readability are not acceptable merely because geometry does not overlap. | `WORKSPACE-COMPOSITION-002`, then `TABLE-PHYSICALITY-003` |
| RET-ONBOARDING-001 | RETURN-SOON | Welcome / Help / Guide | Riverline opens without sufficient major-job orientation, help/tutorial discovery is buried, and Guide needs a current-product review. Preserve versioned workspace tutorial behavior; do not add a recurring splash/modal. | `WELCOME-INTRO-001`, `SETTINGS-IA-001`, then Guide closeout |
| RET-TABLE-001 | RETURN-SOON | Hand / Table Presence | `TABLE-PRESENCE-002` implemented adaptive geometry and table hierarchy; `AUDIO-MOTION-001` adds bounded TablePresentation-based card/fold/chip/pot/street consequences without changing poker authority. Current table scale/readability/known-card identity and independent Firefox acceptance remain open; implementation is not visual sign-off. | `UX-REGRESSION-001` → `WORKSPACE-COMPOSITION-002` → `TABLE-PHYSICALITY-003` → closeout |
...
| RET-CARDS-THEMES-001 | RETURN-SOON | Cards / Themes / Layout / Density | Premium Cards, custom themes, picker, layout presets, and density are implemented but current hands-on QA found picker/known-card legibility, theme-edit semantics, weak preset/density value, and Table Focus defects. Do not add variants before fixing defaults and completing human Firefox acceptance. | `UX-REGRESSION-001`, `WORKSPACE-COMPOSITION-002`, `SETTINGS-IA-001`, then closeout |
| RET-AUDIO-001 | RETURN-LATER / ACCEPTED CHECKPOINT DEBT | Audio / Motion | `AUDIO-MOTION-001` is accepted as an implementation checkpoint without a claim of subjective perfection. Return for subjective Study/UI sound refinement, Check refinement if still desired, fatigue review, and any unperformed Firefox visual/audio acceptance. Architecture, routing, motion, and accepted physical configuration remain checkpointed. | later dedicated audio-polish/acceptance pass when prioritized; does not block `DOCS-INTEGRITY-001` |
...
| RET-HOMEGAME-001 | RETURN-SOON | Home Game Organizer | **Create Game currently routes to Home instead of starting/opening a game flow.** `HOME-GAME-001A` also remains incomplete for saved-player edit/archive, visible correction history, archive/delete confirmation, richer management UX, import/export decision, and Firefox acceptance. | immediate blocker: `UX-REGRESSION-001`; broader: `HOME-GAME-001B` |
