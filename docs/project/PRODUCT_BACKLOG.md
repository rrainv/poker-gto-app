...
| 13 | COMPLETED / ACCEPTED FOUNDATIONAL | `MASSIVE-FOUNDATION-CHECKPOINT-001`: Identity Lifecycle A/B/C, truth/assessment gates, first Training Intelligence, and Personal Strategy Intelligence |
| 14 | ACTIVE NEXT / FOUNDATIONAL | `BROWSER-TEST-PLATFORM-001` |
...
| 22 | PLANNED NEXT | learning-loop expansion after foundation gates |
| 23 | COMPLETED / ACCEPTED FOUNDATION | `PERSONAL-STRATEGY-INTELLIGENCE-001`; Coach and observed-intent integration remain preserved future capability work |
...
The **September Alpha** milestone and independent-audit checkpoint are complete as evidence capture and human triage, not public-beta readiness. The resulting foundation gates do not reopen accepted UI checkpoints wholesale, but they supersede claims that owner indexes, heuristic agreement, source metadata, structural browser substitutes, or a bounded Saved result set are sufficient product closure.
...
| `HOME-HAND-LIFECYCLE-001` | COMPLETED / HUMAN ACCEPTED | Home treats only non-terminal canonical Hands as resumable. Completed showdown, fold, and all-in Hands are not presented as live or continuable. |
...
|---|---|---|
| source authority, DecisionContext, fallback repair, role semantics, benchmark, and bounded calibration | CHECKPOINTED / TRUST, ECONOMICS, AND CONTEXT CONVERGENCE ACCEPTED | One `DecisionContext → StrategyProvider → StrategyResult → StrategyClaimPolicy` path is established. `DECISION-ECONOMICS-001` preserves canonical accounting and exact actor-relative pricing only where evidence exists. `STRATEGY-TRUST-001` requires provider declaration → structural/source validation → application-owned acceptance → effective bounded authority. `DECISION-CONTEXT-SINGLE-AUTHORITY-001` makes `deriveDecisionContextFromPlaybookScenario()` the Scenario projector, `deriveDecisionContextFromPokerState()` the Hand projector, and `resolvePlaybookDecisionContext()` their selector; missing or failed dependencies clear stale state without local reconstruction. Strong claims cannot be self-declared, historical ClaimPolicy evidence remains frozen, and malformed accepted exact/normative distributions fail closed at the shared `1e-12` tolerance. No production Reference Pack exists. Broader `logic.js` composition remains with `UI-COMPOSITION-ROOT-001`. |
| `HEURISTIC-BASELINE-TRUTH-001` | COMPLETED / ACCEPTED FOUNDATION | Shared truth projection constrains the current heuristic to exploratory/comparative baseline evidence. Agreement/disagreement is not skill, correctness, GTO, or automatic remediation. An explicit application-owned assessment policy is required for normative claims. |
...
| Training Memory / DecisionRecord / review and re-drill | CHECKPOINTED / IDENTITY AND FIRST INTELLIGENCE SLICE ACCEPTED | Durable evidence, review lifecycle, Full Hand replay sharing, Same Spot, and Similar Spot remain intact under durable Device Guest/account generation isolation. Optional pre-reveal Unsure evidence and explicit exact 24-hour Revisit are implemented with historical-answer embargo and why-returned facts. Heuristic comparison and normative assessment remain separate authorities. See [Training Memory v1](TRAINING_MEMORY_V1_SPEC.md), [Training Intelligence v1](TRAINING_INTELLIGENCE_V1_SPEC.md), and [Training Intelligence](capabilities/TRAINING_INTELLIGENCE.md). |
...
| `PERSONAL-STRATEGY-INTELLIGENCE-001` / former `003A` foundation | COMPLETED / ACCEPTED FOUNDATION | Arbitrary named Game Setups and Approaches, versioned migration, structural boundary-driven RFI mapping, confirmed qualitative evidence, What Riverline understands, Teach Riverline Next, Matrix Edit, and claim-bounded natural-language range analysis/comparison share one immutable intended-evidence authority. Coach, observed/opponent integration, continuation ranges, and postflop/combo depth remain later slices. |
...
| `SAVED-LIBRARY-001` | PLANNED NEXT / FOUNDATIONAL RETRIEVAL | Full retrieval, search, filtering, sorting, and pagination over the existing Hand/Spot library. It does not authorize new payload kinds. The broader `HOME-002B` master-detail evolution remains later. See [Saved Knowledge and Sharing](capabilities/SAVED_KNOWLEDGE_AND_SHARING.md). |
| `HOME-STUDY-CONTINUITY-001` | PLANNED NEXT | Connect only explicit Hand, Training, Saved, and other study resume/history contracts after ownership and retrieval gates; never fabricate recency or mastery. |
...
| Capability | Status | Accepted scope |
|---|---|---|
| identity/auth plus Saved and Personal Strategy sync | CHECKPOINTED / IDENTITY LIFECYCLE FOUNDATION ACCEPTED | Supabase email/password/profile and explicit opt-in sync foundations share one runtime client. Identity Lifecycle A/B/C add durable Device Guest ownership, cross-surface generation/disposal fences, account isolation, explicit Move/Keep/Cancel, and journaled promotion/recovery. Live provider/RLS acceptance remains separate return debt. |
...
| Capability | Status | Accepted scope |
|---|---|---|
| `HOME-GAME-001A` | CHECKPOINTED / INTENTIONALLY INCOMPLETE | Separate players/groups/sessions, exact integer money, append-only corrections, chips separate from money, lifecycle/reopen, balance rejection, deterministic settlement, account IndexedDB/Guest memory. Home Game remains a supporting utility; expansion freezes after safety fixes. |
