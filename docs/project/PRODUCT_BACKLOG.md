...
| 23 | PLANNED NEXT | `PERSONAL-STRATEGY-003A` |
...
| Training Memory / DecisionRecord / review and re-drill | CHECKPOINTED / AUTH ISOLATION AND MAINTENANCE HUMAN-ACCEPTED | Durable evidence, recent history, review lifecycle, and Full Hand replay sharing remain intact. Auth-aware bridge installation recovers from bootstrap ordering; signed-in local Memory works in idle, Varied, Focused, and Full Hand without Supabase sync while owner isolation remains intact. Same Spot is a standalone idle-only historical re-drill, neither Focused nor planner-backed; active ordinary Training blocks entry without suspend/restore, frozen Earlier/This try and Baseline then/Reference then evidence remains explicit, and ordinary headline statistics/planner progress are unchanged. Similar Spot remains current planner/generator-backed practice. Durable anonymous Device Guest remains future `IDENTITY-LIFECYCLE-001` work. See [Training Memory v1](TRAINING_MEMORY_V1_SPEC.md), [Learning Evidence Foundation](capabilities/LEARNING_EVIDENCE_FOUNDATION.md), and [Training Intelligence](capabilities/TRAINING_INTELLIGENCE.md). |
