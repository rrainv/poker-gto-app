...
The range core is implemented, but no production action-conditioned range adapter or preflop-to-river trajectory exists. Current StrategyProvider resolves one concrete decision/combo at a time and does not supply a complete source-owned range. Current production Analysis attaches no weighted range. Personal Strategy has class-level action evidence/snapshots but no semantics-safe adapter to `HoldemWeightedRange`. Current `equity-request/v1` cannot accept weighted opponents.
