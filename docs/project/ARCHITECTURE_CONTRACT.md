...
All consumers must obtain user-facing claim semantics from `StrategyClaimPolicy v1` and the shared `strategy-truth/v1` projection. Source acceptance and assessment acceptance are separate: `strategy-assessment-policy/v1` binds an explicitly accepted criterion to exact source fingerprint, canonical context, capabilities and action/sizing support. No production normative policy is registered. Historical Training freezes this projection additively and never upgrades from current registries. They must not infer correctness, optimality, exactness, EV loss, normative grading, skill, accuracy, mastery, or GTO from a source ID, family, solver/model branding, probability distribution, agreement, disagreement, or confidence number. The current heuristic is exploratory/comparative baseline evidence only, and its disagreement alone must not create remediation. See `STRATEGY_SOURCE_AUTHORITY_SPEC.md`.
...
`training-decision-record/v1` and `training-session-record/v1` are the durable Training Memory authority for what an owner was shown and answered. They preserve canonical reproduction identity plus the frozen StrategyResult/StrategyClaimPolicy at answer time; they do not re-grade history with today's provider. Review reasons, summaries, priority, and `training-similarity/v1` are deterministic projections. Exact Same Spot uses frozen historical comparison; Similar Spot must route a versioned envelope through the Practice Planner and canonical generator and is labelled current. SavedStudyObject remains intentional bookmarking, while Training Memory remains encountered-decision evidence. Full Hand stores one session-level replay source with per-decision replay references. `AUTH-TRAINING-MEMORY-001` is human/security accepted: every Training Memory operation resolves an authentication-aware owner scope, captures auth/owner generation, and fails closed when that generation becomes stale. Explicit sign-out revokes local access before provider cleanup, queued stale intents are discarded, and in-flight IndexedDB writes are transaction-aborted. Stored account bytes are not deleted and become accessible again only after that account authenticates. Slice B adds durable Device Guest learning storage through lifecycle-owned bindings, including a stable persisted Training Memory owner reference distinct from current authentication class; Slice C promotes a never-bound account through a recoverable registry journal while preserving that persisted owner reference and every domain record. See `TRAINING_MEMORY_V1_SPEC.md`.
