...
- `training-decision-record/v1` / `training-session-record/v1` own durable encountered-decision/session evidence; historical source/claim snapshots are immutable, and Full Hand decisions share one session replay authority. Authentication-aware owner/generation scope now gates every operation, while the durable anonymous Device Guest and generalized cross-surface lifecycle remain future `IDENTITY-LIFECYCLE-001` work.
