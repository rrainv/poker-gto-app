...
Randomization is a shared restrained interaction language, not a universal poker-state generator. Each surface keeps its own application adapter, commit boundary, and state authority; only bounded deterministic/recipe infrastructure is shared where appropriate.

`Keep` means preserve the named semantic component while a requested randomization changes only the supported non-kept components. `Change only` targets exactly one supported component. Both respect canonical card identity, exclusions, visibility, legality, and surface readiness. A consumer cannot interpret Keep as persistence, Saved state, a strategy constraint, or permission to rewrite canonical history. Randomness carries no GTO, strategic-importance, population-frequency, or representative-spot claim; recipe/version details remain secondary unless requested.
...
| Randomize/Keep | APPLICABLE NOW only to the pending uncommitted chance draft; never committed Hand history or opponent private cards | APPLICABLE NOW through the owning lossy Scenario adapter and readiness gate | NOT APPLICABLE to the canonical Training generator's private curriculum contract | APPLICABLE NOW through the owning Equity-input adapter; no automatic calculation | NOT APPLICABLE to ordinary evidence editing | NOT APPLICABLE to passive inspection | NOT APPLICABLE to money/session records |
