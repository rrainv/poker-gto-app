...
| QA-REVIEW-NAV-001 | DEFERRED / PRODUCT DESIGN | Compare the current horizontal Action Path/timeline with a vertical chronological navigator for scanability, street grouping, current-decision visibility, direct Replay seeking, long-hand and 1080p efficiency, Deep Hand Review fit, EN/RU/HE, RTL, and keyboard access. Horizontal orientation is current implementation, not a permanently accepted competitive-reference decision. | `RET-REVIEW-NAV-001` / dedicated Hand Review-Replay design pass |
...
| QA-HANDSON-007 | OPEN | Guide needs a current-product content review. | later `PREMIUM-CLOSEOUT-001` pre-release Guide finalization |
...
| QA-HANDSON-017 | PARTIAL / ACCEPTED CHECKPOINT | Compact Equity player tiles, horizontal multi-player density, and the same tile language through input/running/completed states are accepted; do not restore old horizontal result blocks. The tile region still expands too aggressively across wide desktops and needs a bounded, left-anchored input allocation that preserves practical access to Board, Dead Cards, method, and shared results. | `RET-COMPOSITION-002` later Equity composition refinement |
