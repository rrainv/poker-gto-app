...
Last consolidated: August 25, 2026 (`WORKSPACE-COMPOSITION-002` accepted as an implementation checkpoint with known composition and manual-verification debt).
...
| QA-HANDSON-015 | PARTIAL | Workspace/table scale and Analyze anchoring improved at the accepted composition checkpoint. Felt, rail, seat, chip, pot, and deeper physical aesthetics remain `TABLE-PHYSICALITY-003`, which starts only after `DOCS-CAPABILITY-DOSSIERS-001`. | `TABLE-PHYSICALITY-003` after the documentation checkpoint |
...
| QA-HANDSON-021 | PARTIAL / IMPLEMENTATION REPAIRED | The implementation restores a perceptual rank+suit floor for Hero and revealed opponents across 2-/4-color and T/10 variants while keeping hidden cards hidden, with automated and Firefox coverage. Independent final human verification across the relevant states was not completed, so the permanent known-card rank+suit invariant remains manual acceptance debt. | `RET-COMPOSITION-002` / later human card verification |
